...
def auto_trader_config_path_for_owner(owner_id: str | None = None, *, root: str | None = None) -> str:
    ...
    base = root or _ROOT
    if not owner:
        return os.path.join(base, "api", "auto_trader_config.json")
    return os.path.join(base, "data", "auto_trader", owner, "auto_trader_config.json")
...
def _auto_trader_config_path() -> str:
    env_path = str(os.getenv("AUTO_TRADER_CONFIG_PATH") or "").strip()
    if env_path:
        return env_path
    return auto_trader_config_path_for_owner(_auto_trader_owner_id())
...
class AutoTraderService:
    ...
    def _save_config(self) -> None:
        try:
            parent = os.path.dirname(self._config_path)
            if parent:
    ...
    def _save_backups(self, rows: list[dict[str, Any]]) -> None:
        try:
            parent = os.path.dirname(self._backup_path)
            if parent:
                os.makedirs(parent, exist_ok=True)
    ...
    def _execute_trade_with_risk_control(
        ...
        if not _auto_trader_owner_id() or not _auto_trader_account_id() or not _auto_trader_broker_provider():
            return {
                ...
                "reason": "missing_explicit_account_context",
