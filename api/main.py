...
QQQ_0DTE_LIVE_WORKER_MANUAL_REVIEW_LOCK_FILE = os.path.join(ROOT, "data", "qqq_0dte", "live_worker_manual_review_lock.json")
QQQ_0DTE_LIVE_WORKER_ORDER_LIFECYCLE_FILE = os.path.join(ROOT, "data", "qqq_0dte", "live_worker_order_lifecycle.jsonl")
...
QQQ_1DTE_LIVE_WORKER_MANUAL_REVIEW_LOCK_FILE = os.path.join(ROOT, "data", "qqq_1dte", "live_worker_manual_review_lock.json")
QQQ_1DTE_LIVE_WORKER_ORDER_LIFECYCLE_FILE = os.path.join(ROOT, "data", "qqq_1dte", "live_worker_order_lifecycle.jsonl")
STOCK_OPTIONS_SWING_WORKER_PID_FILE = os.path.join(ROOT, ".stock_options_swing_worker.pid")
STOCK_OPTIONS_SWING_WORKER_STOP_FILE = os.path.join(ROOT, ".stock_options_swing_worker.stop")
STOCK_OPTIONS_SWING_WORKER_RUNTIME_FILE = os.path.join(ROOT, ".stock_options_swing_worker.runtime.json")
...
def _frontend_cors_origins() -> list[str]:
    ...
    origins.update(s.strip() for s in raw.split(",") if s.strip())
    try:
        ...
        for host in list(hosts):
            ...
            for info in socket.getaddrinfo(host, None, family=socket.AF_INET):
                ...
                if ip and not ip.startswith("127."):
                    origins.add(f"http://{ip}:{web_port}")
    except Exception:
        pass
    return sorted(origins)


CORS_ALLOW_ORIGINS = _frontend_cors_origins()
...
def _terminate_process_tree(pid: Optional[int], timeout_seconds: float = 1.0) -> bool:
    if not pid or not is_pid_alive(pid):
        return False
    if os.name == "nt":
        try:
            ...
            if proc.returncode != 0 and is_pid_alive(pid):
                return False
            ...
            return False
    return _terminate_pid(pid, timeout_seconds)
...
def _cleanup_orphan_auto_trader_processes(timeout_seconds: float = 1.0) -> int:
    total = 0
    seen: set[int] = set()
    for name in ("auto_trader_supervisor.py", "auto_trader_worker.py"):
        for pid in _list_python_pids_by_script(name):
            if pid in seen:
                continue
            seen.add(pid)
            if _is_auto_trader_script_pid(pid) and _terminate_process_tree(pid, timeout_seconds):
                total += 1
    return total
...
def _cleanup_orphan_stock_options_swing_worker_processes() -> int:
    total = 0
    for pid in _list_python_pids_by_script("stock_options_swing_worker.py"):
        if _terminate_pid(pid):
            total += 1
    return total
...
def _current_stock_options_swing_worker_pid() -> Optional[int]:
    pid = read_pid_file(STOCK_OPTIONS_SWING_WORKER_PID_FILE)
    if is_pid_alive(pid) and _is_stock_options_swing_worker_script_pid(pid):
        return pid
    running = _list_python_pids_by_script("stock_options_swing_worker.py")
    if running:
        chosen = int(sorted(running)[0])
        _write_pid_file(STOCK_OPTIONS_SWING_WORKER_PID_FILE, chosen)
        return chosen
    return None
...
def _start_auto_trader_worker(owner_id: str | None = None) -> str:
    ...
    with _AUTO_TRADER_PROCESS_OP_LOCK:
        ...
        if owner:
            env["AUTO_TRADER_CONFIG_PATH"] = _auto_trader_config_path_for_owner(owner)
...
def _stop_auto_trader_worker(timeout_seconds: float = 5.0) -> str:
    with _AUTO_TRADER_PROCESS_OP_LOCK:
        stopped_any = False
        stop_wait_seconds = max(0.5, min(float(timeout_seconds or 5.0), 1.5))
        _remove_file_silent(AUTO_TRADER_SUPERVISOR_STOP_FILE)
        try:
        ...
        if p is not None:
            if p.poll() is None:
                try:
                    ...
                    if _is_auto_trader_script_pid(p.pid):
                        stopped_any = _terminate_process_tree(p.pid, stop_wait_seconds) or stopped_any
                    pass
        ...
        for pid in candidate_pids:
            ...
            if _is_auto_trader_script_pid(pid):
                stopped_any = _terminate_process_tree(pid, stop_wait_seconds) or stopped_any
        if _cleanup_orphan_auto_trader_processes(stop_wait_seconds) > 0:
            stopped_any = True
        # 再做一次兜底清理，覆盖 supervisor 刚退出后短暂拉起子进程的竞态窗口。
        if _cleanup_orphan_auto_trader_processes(stop_wait_seconds) > 0:
            stopped_any = True
        if not _list_python_pids_by_script("auto_trader_worker.py"):
            _remove_file_silent(AUTO_TRADER_WORKER_RUNTIME_FILE)
            _remove_file_silent(AUTO_TRADER_PID_FILE)
        if not _list_python_pids_by_script("auto_trader_supervisor.py"):
...
def _tail_jsonl(path: str | None, limit: int = 8) -> list[dict[str, Any]]:
    if not path or not os.path.isfile(path):
        return []
    try:
        with open(path, "r", encoding="utf-8") as f:
            lines = f.readlines()[-max(1, min(int(limit or 8), 100)) :]
    except Exception:
        return []
...
def _row_matches_account_context(row: dict[str, Any] | None, context: dict[str, str] | None) -> bool:
...
def _runtime_matches_account_context(runtime: dict[str, Any] | None, context: dict[str, str] | None) -> bool:
    if not isinstance(runtime, dict):
        return True
    ctx = context if isinstance(context, dict) else {}
    inner = runtime.get("runtime") if isinstance(runtime.get("runtime"), dict) else {}
    for key in ("owner_id", "account_id", "broker_provider"):
        current = str(ctx.get(key) or "").strip().lower()
        got = str(runtime.get(key) or inner.get(key) or "").strip().lower()
        if current and (not got or got != current):
            return False
    return True


def _qqq_live_order_lifecycle_summary(path: str | None, account_context: dict[str, str] | None = None) -> dict[str, Any] | None:
    events = [row for row in _tail_jsonl(path, 100) if _row_matches_account_context(row, account_context)][-8:]
    if not events:
        return None
    ...
    latest = events[-1]
    latest_state = str(latest.get("state") or "").strip()
    ...
    return {
        ...
        "latest_at": str(latest.get("at") or "").strip() or None,
        "recent": events,
        "by_state": by_state,
        ...
        "severity": severity,
        "attention_reasons": [reason] if reason else [],
    }


def _manual_review_lock_matches_context(lock: dict[str, Any] | None, context: dict[str, str] | None) -> bool:
...
def _qqq_live_runtime_status(
    runtime_file: str,
    pid_file: str,
    manual_review_lock_file: str | None = None,
    order_lifecycle_file: str | None = None,
    account_context: dict[str, str] | None = None,
) -> dict[str, Any]:
    snap = _read_json_file(runtime_file)
    runtime = snap if isinstance(snap, dict) else {}
    if not _runtime_matches_account_context(runtime, account_context):
        runtime = {
        ...
        lifecycle = _qqq_live_order_lifecycle_summary(order_lifecycle_file, account_context)
        if lifecycle:
            summary = runtime.get("intraday_safety_summary") if isinstance(runtime.get("intraday_safety_summary"), dict) else {}
            runtime = {**runtime, "intraday_safety_summary": {**summary, "order_lifecycle": lifecycle}}
        lock = _read_json_file(manual_review_lock_file) if manual_review_lock_file else None
        if isinstance(lock, dict) and bool(lock.get("locked", True)) and _manual_review_lock_matches_context(lock, account_context):
            summary = runtime.get("intraday_safety_summary") if isinstance(runtime.get("intraday_safety_summary"), dict) else {}
            runtime = {
                **runtime,
                "intraday_safety_summary": {
                    **summary,
                    "manual_review_locked": True,
                    "manual_review_reason": lock.get("reason"),
                    "manual_review_created_at": lock.get("created_at"),
                    "manual_review_order_ids": lock.get("order_ids") if isinstance(lock.get("order_ids"), list) else [],
                    "manual_review_expected_symbols": lock.get("expected_symbols") if isinstance(lock.get("expected_symbols"), list) else [],
                    "manual_review_lock": lock,
                },
            }
        else:
            summary = runtime.get("intraday_safety_summary") if isinstance(runtime.get("intraday_safety_summary"), dict) else {}
            if summary.get("manual_review_locked") or summary.get("manual_review_lock"):
                runtime = {
                    **runtime,
                    "intraday_safety_summary": {
                        **summary,
                        "manual_review_locked": False,
                        "manual_review_reason": None,
                        "manual_review_created_at": None,
                        "manual_review_order_ids": [],
                        "manual_review_expected_symbols": [],
                        "manual_review_lock": None,
                    },
                }
...
def _qqq_live_status_account_context(instance: str, owner_id: str | None = None) -> dict[str, str]:
    owner = str(owner_id or "").strip().lower()
    ...
    if owner:
        try:
            ...
            cfg_path = rt._qqq_live_worker_config_path(instance, owner)
        except Exception:
            pass
    cfg = _read_json_file(cfg_path)
    cfg_account_id = str((cfg or {}).get("account_id") or "").strip() or None
    ctx = _resolve_worker_account_context(owner, cfg_account_id) if owner else {}
    if owner:
        ctx["owner_id"] = owner
    return ctx


def _qqq_0dte_live_runtime_status(owner_id: str | None = None) -> dict[str, Any]:
    return _qqq_live_runtime_status(
        QQQ_0DTE_LIVE_WORKER_RUNTIME_FILE,
        QQQ_0DTE_LIVE_WORKER_PID_FILE,
        QQQ_0DTE_LIVE_WORKER_MANUAL_REVIEW_LOCK_FILE,
        QQQ_0DTE_LIVE_WORKER_ORDER_LIFECYCLE_FILE,
        _qqq_live_status_account_context("0dte", owner_id),
    )


def _qqq_1dte_live_runtime_status(owner_id: str | None = None) -> dict[str, Any]:
    return _qqq_live_runtime_status(
        QQQ_1DTE_LIVE_WORKER_RUNTIME_FILE,
        QQQ_1DTE_LIVE_WORKER_PID_FILE,
        QQQ_1DTE_LIVE_WORKER_MANUAL_REVIEW_LOCK_FILE,
        QQQ_1DTE_LIVE_WORKER_ORDER_LIFECYCLE_FILE,
        _qqq_live_status_account_context("1dte", owner_id),
    )


def _stock_options_swing_status_account_context(owner_id: str | None = None) -> dict[str, str]:
    owner = str(owner_id or "").strip().lower()
    ...
    if owner:
        try:
            ...
            cfg_path = rt._stock_options_swing_config_path(owner)
        except Exception:
            pass
    cfg = _read_json_file(cfg_path)
    cfg_account_id = str((cfg or {}).get("account_id") or "").strip() or None
...
def _stock_options_swing_runtime_status(owner_id: str | None = None) -> dict[str, Any]:
    snap = _read_json_file(STOCK_OPTIONS_SWING_WORKER_RUNTIME_FILE)
    runtime = snap if isinstance(snap, dict) else {}
    account_context = _stock_options_swing_status_account_context(owner_id)
    if not _runtime_matches_account_context(runtime, account_context):
        runtime = {
            "status": "context_mismatch",
            "context_mismatch": True,
    ...
    pid = _current_stock_options_swing_worker_pid()
    alive = bool(pid and is_pid_alive(pid))
    status = str(runtime.get("status") or "").strip().lower()
    if not alive:
        state = "stopped"
...
def _start_stock_options_swing_worker(owner_id: str | None = None) -> str:
    ...
    if owner:
        try:
            ...
            cfg_path = os.path.join(ROOT, "data", "stock_options_swing", "live_worker_config.json")
    with _QQQ_LIVE_WORKERS_PROCESS_OP_LOCK:
        existing = _current_stock_options_swing_worker_pid()
        if existing and _is_stock_options_swing_worker_script_pid(existing):
            return "already_running"
        p = _managed_processes.get(managed_key)
        if p and p.poll() is None:
            return "already_running"
        pid = read_pid_file(STOCK_OPTIONS_SWING_WORKER_PID_FILE)
        if is_pid_alive(pid):
            if _is_stock_options_swing_worker_script_pid(pid):
                return "already_running"
            _remove_file_silent(STOCK_OPTIONS_SWING_WORKER_PID_FILE)
        ...
        child = _managed_processes.get(managed_key)
        if child is not None and getattr(child, "pid", None):
            _write_pid_file(STOCK_OPTIONS_SWING_WORKER_PID_FILE, int(child.pid))
        time.sleep(0.25)
        child = _managed_processes.get(managed_key)
        if child is not None and child.poll() is not None:
            code = int(child.poll() or 0)
...
def _stop_stock_options_swing_worker(timeout_seconds: float = 5.0) -> str:
    managed_key = "stock_options_swing_worker"
    with _QQQ_LIVE_WORKERS_PROCESS_OP_LOCK:
        stopped_any = False
        try:
            with open(STOCK_OPTIONS_SWING_WORKER_STOP_FILE, "w", encoding="utf-8") as f:
                f.write(datetime.now().isoformat())
        except Exception:
            pass
        p = _managed_processes.get(managed_key)
        if p is not None:
            if p.poll() is None:
                try:
                    ...
                    pass
            _managed_processes.pop(managed_key, None)
        wpid = read_pid_file(STOCK_OPTIONS_SWING_WORKER_PID_FILE)
        if _is_stock_options_swing_worker_script_pid(wpid):
            stopped_any = _terminate_pid(wpid, timeout_seconds) or stopped_any
        if _cleanup_orphan_stock_options_swing_worker_processes() > 0:
            stopped_any = True
        if not is_pid_alive(read_pid_file(STOCK_OPTIONS_SWING_WORKER_PID_FILE)):
            _remove_file_silent(STOCK_OPTIONS_SWING_WORKER_RUNTIME_FILE)
        _remove_file_silent(STOCK_OPTIONS_SWING_WORKER_STOP_FILE)
        return "stopped" if stopped_any else "not_running"
...
_AUTO_TRADER_SERVICE_BY_OWNER_LOCK = threading.RLock()
_AUTO_TRADER_SERVICE_BY_OWNER: dict[str, AutoTraderService] = {}


def _normalize_runtime_owner(owner_id: str | None = None) -> str:
    return str(owner_id or "").strip().lower()


def _auto_trader_config_path_for_owner(owner_id: str | None = None) -> str:
    return auto_trader_config_path_for_owner(_normalize_runtime_owner(owner_id), root=ROOT)


def _auto_trader_service_for_owner(owner_id: str | None = None) -> AutoTraderService:
    owner = _normalize_runtime_owner(owner_id)
    if not owner:
        return auto_trader
    with _AUTO_TRADER_SERVICE_BY_OWNER_LOCK:
        svc = _AUTO_TRADER_SERVICE_BY_OWNER.get(owner)
        if svc is not None:
            return svc
        svc = AutoTraderService(
            ...
            get_positions=_get_positions_for_auto_trader,
            get_account=_get_account_for_auto_trader,
            config_path=_auto_trader_config_path_for_owner(owner),
        )
        _AUTO_TRADER_SERVICE_BY_OWNER[owner] = svc
        return svc
    ...
    if rows:
        out = {
            ...
            "diagnostics": {
                ...
                "no_signal_diagnostics": summary.get("no_signal_diagnostics", {}) if isinstance(summary, dict) else {},
...
def auto_trader_scan_run(owner_id: str | None = None) -> dict[str, Any]:
    started = time.perf_counter()
    account_context = _auto_trader_signal_account_context(owner_id)
    runtime = _auto_trader_runtime_status()
    if account_context and not _runtime_matches_account_context(runtime, account_context):
        emit_metric(
            event="api.auto_trader.scan_run",
            ok=False,
            elapsed_ms=(time.perf_counter() - started) * 1000.0,
            extra={"error": "worker_account_context_mismatch", "account_context": account_context},
        )
        raise HTTPException(
            status_code=403,
            detail={
                "error": "worker_account_context_mismatch",
                "message": "current_worker_belongs_to_different_owner_account_context",
...
def _auto_trader_signal_account_context(owner_id: str | None = None) -> dict[str, str]:
    owner = str(owner_id or "").strip().lower()
    ctx = _resolve_worker_account_context(owner) if owner else {}
    if owner:
        ctx["owner_id"] = owner
    return ctx
...
def auto_trader_signals(status: str = "all", owner_id: str | None = None) -> dict[str, Any]:
    ...
    mem_items = [
        s
        for s in auto_trader.list_signals(status=status)
        if _row_matches_account_context(s if isinstance(s, dict) else None, account_context)
    ]
    disk_items = [
        s
        for s in load_persisted_signals(status=status)
        if _row_matches_account_context(s if isinstance(s, dict) else None, account_context)
    ]
    ...
    return {"status": status, "items": items, "account_context": account_context or None}
        ...
        with _AUTO_TRADER_CONFIRM_QUEUE_LOCK:
                            ...
                            for x in sigs:
                                ...
                                if sid:
                                    entries.append(
                                        {
                                            "signal_id": sid,
                                            "confirmation_token": queued_token,
            ...
            for item in entries:
                ...
                if sid == signal_id:
                    ...
                    for key in ("owner_id", "account_id", "broker_provider"):
                        if account_context and account_context.get(key):
                            item[key] = account_context[key]
                seen.add(sid)
                ...
                compacted.append(
                    {
                        ...
                        **{k: v for k, v in (account_context or {}).items() if v},
                    }
                )
...
def auto_trader_confirm(signal_id: str, body: AutoTraderConfirmBody, owner_id: str | None = None) -> dict[str, Any]:
    runtime = _auto_trader_runtime_status()
    ...
    account_context = _auto_trader_signal_account_context(owner_id)
    signals = auto_trader_signals(status="all", owner_id=owner_id).get("items")
    scoped_signal = None
    if isinstance(signals, list):
        for item in signals:
            if isinstance(item, dict) and str(item.get("signal_id") or "") == str(signal_id or ""):
                scoped_signal = item
                break
    if owner_id and scoped_signal is None:
        raise HTTPException(
            status_code=404,
            detail={"error": "signal_not_found_for_current_account", "signal_id": signal_id, "account_context": account_context},
        )

    _enqueue_worker_confirm(signal_id, body.confirmation_token, account_context)
    ...
    while time.time() < deadline:
        ...
        for s in rows:
            if str(s.get("signal_id") or "") != str(signal_id or ""):
                continue
            if not _row_matches_account_context(s, account_context):
                continue
            st = s.get("status")
