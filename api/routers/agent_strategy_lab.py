...
from api.services.agent_strategy_lab_service import (
    ...
    create_lab_task,
    create_lab_run,
    create_revalidate_candidate_task,
    get_lab_task,
    get_lab_run,
    lab_status,
    list_approvals,
...
@router.post("/runs/{run_id}/candidates/{candidate_id}/revalidate")
def agent_strategy_lab_candidate_revalidate(
    run_id: str,
    candidate_id: str,
    body: dict[str, Any] = Body(default_factory=dict),
) -> dict[str, Any]:
    try:
        return create_revalidate_candidate_task(run_id, candidate_id, body if isinstance(body, dict) else {})
    except AgentStrategyLabError as exc:
        raise _http_error(exc) from exc
