...
def _ensure_builtin_broker_profiles(d: dict[str, Any]) -> bool:
    br = d.get("brokers")
    if not isinstance(br, dict):
        d["brokers"] = {}
        br = d["brokers"]
    changed = False
    for bid, display_name in BUILTIN_BROKER_DISPLAY_NAMES.items():
        meta = br.get(bid)
        ...
        if not str(meta.get("display_name") or "").strip():
            ...
            changed = True
        if not isinstance(meta.get("schedule"), dict):
            meta["schedule"] = get_default_fee_schedule()
            changed = True
    _ensure_manual_fee_broker_id(d)
    return changed
...
def init_fee_broker_profiles(file_path: str) -> None:
    ...
    builtin_changed = _ensure_builtin_broker_profiles(migrated)
    if raw != migrated or builtin_changed:
