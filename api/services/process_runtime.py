...
def build_setup_services_status(
    *,
    ...
    stock_options_swing_worker_pid_file: str | None = None,
    stock_options_swing_runtime: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """
    ...
    swing_runtime = stock_options_swing_runtime if isinstance(stock_options_swing_runtime, dict) else {}
    swing_running = False
    swing_tracking = "none"
    swing_pid_out: Optional[int] = None
    if stock_options_swing_worker_pid_file:
        swing_running, swing_tracking, swing_pid_out = managed_subprocess_status(
            managed_processes.get("stock_options_swing_worker"), stock_options_swing_worker_pid_file
        )
    ...
    sw_wr = bool(swing_runtime.get("worker_running"))
    sw_wp = swing_runtime.get("pid")
    sw_wp_int: Optional[int] = None
    if sw_wp is not None and str(sw_wp).strip().isdigit():
        sw_wp_int = int(str(sw_wp).strip())
    if sw_wp_int is None and stock_options_swing_worker_pid_file:
        sw_wp_int = read_pid_file(stock_options_swing_worker_pid_file)
    sw_alive = is_pid_alive(sw_wp_int) if sw_wp_int else False
    sw_display_running = bool(swing_running or sw_alive or sw_wr)
    sw_track_out = str(swing_tracking)
    if sw_track_out == "none" and sw_alive:
        sw_track_out = "pid_file"
    elif sw_track_out == "none" and sw_wr:
        sw_track_out = "runtime"
    sw_pid_disp: Optional[int] = swing_pid_out if swing_pid_out else sw_wp_int
    ...
    return {
        ...
        "stock_options_swing_running": sw_display_running,
        "stock_options_swing_tracking": sw_track_out,
        "stock_options_swing_pid": sw_pid_disp if sw_display_running else None,
        "stock_options_swing_runtime": swing_runtime,
    }
