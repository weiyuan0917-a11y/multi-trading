  ...
  const restartWorker = async () => {
    ...
    try {
      ...
      try {
        await apiPost<any>(
          "/setup/services/stop",
          { stop_auto_trader: true, stop_feishu_bot: false },
          { timeoutMs: 45000, retries: 0 }
        );
      } catch (stopErr: any) {
        if (!isTransientRequestError(stopErr)) throw stopErr;
        setSoftNotice("停止 Worker 响应较慢，已进入状态确认。");
      }
      ...
      if (!stopped) {
        throw new Error("停止 Worker 超时，请稍后刷新状态确认。");
      }

      try {
        await apiPost<any>(
          "/setup/services/start",
          { enable_auto_trader: true, start_feishu_bot: false },
          { timeoutMs: 45000, retries: 0 }
        );
      } catch (startErr: any) {
        if (!isTransientRequestError(startErr)) throw startErr;
        const st = await apiGet<any>("/auto-trader/status", { timeoutMs: 8000, retries: 0, cacheTtlMs: 0 }).catch(() => null);
        if (!st?.running) throw startErr;
        setSoftNotice("启动 Worker 响应较慢，但状态已确认运行。");
      }
