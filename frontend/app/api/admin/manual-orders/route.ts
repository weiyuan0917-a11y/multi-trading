...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await callConvex(`/billing/manual-orders?${params.toString()}`);
...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await callConvex("/billing/manual-order-admin", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        ...body,
        confirmedBy: body?.confirmedBy || admin.user?.username || admin.user?.email || "admin",
      }),
    });
    return json(200, result);
  } catch (err: any) {
