...
function formatBacktestDirectionStrike(t: Record<string, unknown>): string {
  ...
  const fmtDoubleK = (x: unknown) =>
    x != null && Number.isFinite(Number(x)) ? Number(x).toFixed(2) : "-";
  if (side === "double_strangle") {
    const legs = t.double_strangle_legs;
    if (legs && typeof legs === "object") {
      const legMap = legs as Record<string, Record<string, unknown> | undefined>;
      return [
        "双宽跨",
        `CL=${fmtDoubleK(legMap.call_long?.strike)}`,
        `CS=${fmtDoubleK(legMap.call_short?.strike)}`,
        `PL=${fmtDoubleK(legMap.put_long?.strike)}`,
        `PS=${fmtDoubleK(legMap.put_short?.strike)}`,
      ].join(" · ");
    }
    return "双宽跨";
  }
...
const MATRIX_PARAM_DEFS: readonly {
  ...
  {
    key: "double_strangle_combo_take_profit_pct_ui",
    labelZh: "双宽跨：四腿组合止盈（%）",
    hint: "四条腿合计权利金止盈，60 表示 +60%。",
    defaultValuesText: "45, 60, 80",
    defaultEnabled: false,
  },
  {
    key: "double_strangle_combo_stop_loss_pct_ui",
    labelZh: "双宽跨：四腿组合止损（%）",
    hint: "四条腿合计权利金止损，30 表示 -30%；0 表示关闭。",
    defaultValuesText: "0, 25, 35",
    defaultEnabled: false,
  },
  {
    key: "double_strangle_single_leg_stop_loss_pct_ui",
    labelZh: "双宽跨：单腿止损（%）",
    hint: "不区分长短腿，任一腿相对自身成本亏损达到阈值则平该腿。",
    defaultValuesText: "25, 35, 45",
    defaultEnabled: false,
  },
  {
    key: "double_strangle_call_long_leg_take_profit_pct_ui",
    labelZh: "双宽跨：Call 长腿单腿止盈（%）",
    hint: "Call 长腿相对自身开仓成本的止盈比例；100 表示 +100%。",
    defaultValuesText: "80, 100, 130",
    defaultEnabled: false,
  },
  {
    key: "double_strangle_call_short_leg_take_profit_pct_ui",
    labelZh: "双宽跨：Call 短腿单腿止盈（%）",
    hint: "Call 短腿相对自身开仓成本的止盈比例；35 表示 +35%。",
    defaultValuesText: "25, 35, 50",
    defaultEnabled: false,
  },
  {
    key: "double_strangle_put_long_leg_take_profit_pct_ui",
    labelZh: "双宽跨：Put 长腿单腿止盈（%）",
    hint: "Put 长腿相对自身开仓成本的止盈比例；100 表示 +100%。",
    defaultValuesText: "80, 100, 130",
    defaultEnabled: false,
  },
  {
    key: "double_strangle_put_short_leg_take_profit_pct_ui",
    labelZh: "双宽跨：Put 短腿单腿止盈（%）",
    hint: "Put 短腿相对自身开仓成本的止盈比例；35 表示 +35%。",
    defaultValuesText: "25, 35, 50",
    defaultEnabled: false,
  },
  {
    key: "double_strangle_call_long_strikes_otm",
    labelZh: "双宽跨：Call 长腿 OTM 档数",
    hint: "必须大于 Call 短腿档数。",
    defaultValuesText: "2, 3",
    defaultEnabled: false,
  },
  {
    key: "double_strangle_call_short_strikes_otm",
    labelZh: "双宽跨：Call 短腿 OTM 档数",
    hint: "靠近 ATM 的 Call 腿。",
    defaultValuesText: "1, 2",
    defaultEnabled: false,
  },
  {
    key: "double_strangle_put_long_strikes_otm",
    labelZh: "双宽跨：Put 长腿 OTM 档数",
    hint: "必须大于 Put 短腿档数。",
    defaultValuesText: "2, 3",
    defaultEnabled: false,
  },
  {
    key: "double_strangle_put_short_strikes_otm",
    labelZh: "双宽跨：Put 短腿 OTM 档数",
    hint: "靠近 ATM 的 Put 腿。",
    defaultValuesText: "1, 2",
    defaultEnabled: false,
  },
...
type MatrixStrategyVariant = "reaction_zone" | "morning_strangle" | "morning_double_strangle" | "morning_directional" | "gamma_scalping" | "gamma_pro";
...
const MATRIX_KEYS_MORNING_DOUBLE_STRANGLE = new Set<string>([
  "strangle_range_pct_ui",
  "strangle_stop_loss_cooldown_minutes",
  ...
  "double_strangle_call_long_leg_take_profit_pct_ui",
  "double_strangle_call_short_leg_take_profit_pct_ui",
  "double_strangle_put_long_leg_take_profit_pct_ui",
  "double_strangle_put_short_leg_take_profit_pct_ui",
  "double_strangle_single_leg_stop_loss_pct_ui",
  "double_strangle_combo_take_profit_pct_ui",
  "double_strangle_combo_stop_loss_pct_ui",
]);
...
function matrixKeysForVariant(v: MatrixStrategyVariant): Set<string> {
  ...
  if (v === "morning_double_strangle") return MATRIX_KEYS_MORNING_DOUBLE_STRANGLE;
...
type StrategyFormState = {
  strategy_variant: "reaction_zone" | "morning_strangle" | "morning_double_strangle" | "morning_directional" | "gamma_scalping" | "gamma_pro";
  ...
  double_strangle_call_long_strikes_otm: number;
  double_strangle_call_short_strikes_otm: number;
  double_strangle_put_long_strikes_otm: number;
  double_strangle_put_short_strikes_otm: number;
  double_strangle_call_long_leg_take_profit_pct_ui: number;
  double_strangle_call_short_leg_take_profit_pct_ui: number;
  double_strangle_put_long_leg_take_profit_pct_ui: number;
  double_strangle_put_short_leg_take_profit_pct_ui: number;
  double_strangle_single_leg_stop_loss_pct_ui: number;
  double_strangle_combo_take_profit_pct_ui: number;
  double_strangle_combo_stop_loss_pct_ui: number;
  double_strangle_max_total_debit: number;
...
const DEFAULT_FORM: StrategyFormState = {
  ...
  double_strangle_call_long_leg_take_profit_pct_ui: 100,
  double_strangle_call_short_leg_take_profit_pct_ui: 35,
  double_strangle_put_long_leg_take_profit_pct_ui: 100,
  double_strangle_put_short_leg_take_profit_pct_ui: 35,
  double_strangle_single_leg_stop_loss_pct_ui: 35,
  double_strangle_combo_take_profit_pct_ui: 60,
  double_strangle_combo_stop_loss_pct_ui: 30,
  double_strangle_max_total_debit: 0,
...
function formToStrategyConfig(f: StrategyFormState): Record<string, unknown> {
  return {
    ...
    double_strangle_call_long_strikes_otm: Math.max(1, Math.floor(f.double_strangle_call_long_strikes_otm)),
    double_strangle_call_short_strikes_otm: Math.max(0, Math.floor(f.double_strangle_call_short_strikes_otm)),
    double_strangle_put_long_strikes_otm: Math.max(1, Math.floor(f.double_strangle_put_long_strikes_otm)),
    double_strangle_put_short_strikes_otm: Math.max(0, Math.floor(f.double_strangle_put_short_strikes_otm)),
    double_strangle_call_long_leg_take_profit_pct: Math.max(0, f.double_strangle_call_long_leg_take_profit_pct_ui / 100),
    double_strangle_call_short_leg_take_profit_pct: Math.max(0, f.double_strangle_call_short_leg_take_profit_pct_ui / 100),
    double_strangle_put_long_leg_take_profit_pct: Math.max(0, f.double_strangle_put_long_leg_take_profit_pct_ui / 100),
    double_strangle_put_short_leg_take_profit_pct: Math.max(0, f.double_strangle_put_short_leg_take_profit_pct_ui / 100),
    double_strangle_single_leg_stop_loss_pct: Math.max(0, f.double_strangle_single_leg_stop_loss_pct_ui / 100),
    double_strangle_combo_take_profit_pct: Math.max(0, f.double_strangle_combo_take_profit_pct_ui / 100),
    double_strangle_combo_stop_loss_pct: Math.max(0, f.double_strangle_combo_stop_loss_pct_ui / 100),
    double_strangle_max_total_debit: Math.max(0, f.double_strangle_max_total_debit),
    double_strangle_require_all_legs_filled: true,
...
const STRATEGY_CONFIG_BACKEND_DEFAULTS: Record<string, unknown> = {
  ...
  double_strangle_put_long_strikes_otm: 2,
  double_strangle_put_short_strikes_otm: 1,
  double_strangle_call_long_leg_take_profit_pct: 1.0,
  double_strangle_call_short_leg_take_profit_pct: 0.35,
  double_strangle_put_long_leg_take_profit_pct: 1.0,
  double_strangle_put_short_leg_take_profit_pct: 0.35,
  double_strangle_single_leg_stop_loss_pct: 0.35,
  double_strangle_combo_take_profit_pct: 0.6,
  double_strangle_combo_stop_loss_pct: 0.3,
  double_strangle_max_total_debit: 0.0,
  double_strangle_require_all_legs_filled: true,
    ...
    try {
      ...
      if (raw) {
        ...
        if (
          ...
          p.matrixStrategySelect === "morning_double_strangle" ||
            ...
            <div className="flex flex-wrap items-end gap-3 rounded-lg border border-violet-500/25 bg-violet-950/20 px-3 py-2.5">
              <label className="space-y-1">
                ...
                >
                  ...
                  <option value="morning_double_strangle">早盘双宽跨</option>
            ...
            <p className="text-xs text-amber-200/90">
              ...
              {matrixStrategySelect === "reaction_zone"
                ...
                : matrixStrategySelect === "morning_strangle"
                  ? "早盘宽跨"
                  : matrixStrategySelect === "morning_double_strangle"
                    ? "早盘双宽跨"
        ...
        <div className="rounded-lg border border-violet-500/30 bg-violet-950/20 p-3">
          ...
          <label className="space-y-1">
            ...
            >
              ...
              <option value="morning_double_strangle">早盘双宽跨（Call/Put 各两条腿；四腿组合止盈止损 + 分腿止盈）</option>
          ...
          {strategy.strategy_variant === "morning_strangle" || strategy.strategy_variant === "morning_double_strangle" || strategy.strategy_variant === "morning_directional" ? (
            <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <label className="space-y-1">
                <div className="field-label">{strategy.strategy_variant === "morning_strangle" || strategy.strategy_variant === "morning_double_strangle" ? "强制双平" : "强制平仓"}（美东 HH:MM）</div>
                <input
                  className="input-base"
          ...
          {strategy.strategy_variant === "morning_double_strangle" ? (
            <div className="mt-3 grid grid-cols-1 gap-3 border-t border-violet-500/20 pt-3 sm:grid-cols-2 lg:grid-cols-3">
              <NumField label="前收偏离上限（%）" value={strategy.strangle_range_pct_ui} onChange={(n) => patchStrategy({ strangle_range_pct_ui: n })} min={0.01} max={5} step={0.05} />
              <NumField label="Call 长腿 OTM 档数" value={strategy.double_strangle_call_long_strikes_otm} onChange={(n) => patchStrategy({ double_strangle_call_long_strikes_otm: Math.max(1, Math.floor(n)) })} min={1} max={30} step={1} />
              <NumField label="Call 短腿 OTM 档数" value={strategy.double_strangle_call_short_strikes_otm} onChange={(n) => patchStrategy({ double_strangle_call_short_strikes_otm: Math.max(0, Math.floor(n)) })} min={0} max={30} step={1} />
              <NumField label="Put 长腿 OTM 档数" value={strategy.double_strangle_put_long_strikes_otm} onChange={(n) => patchStrategy({ double_strangle_put_long_strikes_otm: Math.max(1, Math.floor(n)) })} min={1} max={30} step={1} />
              <NumField label="Put 短腿 OTM 档数" value={strategy.double_strangle_put_short_strikes_otm} onChange={(n) => patchStrategy({ double_strangle_put_short_strikes_otm: Math.max(0, Math.floor(n)) })} min={0} max={30} step={1} />
              <NumField label="Call 长腿单腿止盈（%）" value={strategy.double_strangle_call_long_leg_take_profit_pct_ui} onChange={(n) => patchStrategy({ double_strangle_call_long_leg_take_profit_pct_ui: n })} min={0} max={500} step={5} />
              <NumField label="Call 短腿单腿止盈（%）" value={strategy.double_strangle_call_short_leg_take_profit_pct_ui} onChange={(n) => patchStrategy({ double_strangle_call_short_leg_take_profit_pct_ui: n })} min={0} max={500} step={5} />
              <NumField label="Put 长腿单腿止盈（%）" value={strategy.double_strangle_put_long_leg_take_profit_pct_ui} onChange={(n) => patchStrategy({ double_strangle_put_long_leg_take_profit_pct_ui: n })} min={0} max={500} step={5} />
              <NumField label="Put 短腿单腿止盈（%）" value={strategy.double_strangle_put_short_leg_take_profit_pct_ui} onChange={(n) => patchStrategy({ double_strangle_put_short_leg_take_profit_pct_ui: n })} min={0} max={500} step={5} />
              <NumField label="单腿止损（%）" value={strategy.double_strangle_single_leg_stop_loss_pct_ui} onChange={(n) => patchStrategy({ double_strangle_single_leg_stop_loss_pct_ui: n })} min={0} max={100} step={1} />
              <NumField label="四腿组合止盈（%）" value={strategy.double_strangle_combo_take_profit_pct_ui} onChange={(n) => patchStrategy({ double_strangle_combo_take_profit_pct_ui: n })} min={0} max={500} step={5} />
              <NumField label="四腿组合止损（%）" value={strategy.double_strangle_combo_stop_loss_pct_ui} onChange={(n) => patchStrategy({ double_strangle_combo_stop_loss_pct_ui: n })} min={0} max={100} step={1} />
              <NumField label="四腿总权利金上限（0=不限制）" value={strategy.double_strangle_max_total_debit} onChange={(n) => patchStrategy({ double_strangle_max_total_debit: Math.max(0, n) })} min={0} max={100} step={0.05} />
              <NumField
                label="组合止损冷静期（分钟）"
                value={strategy.strangle_stop_loss_cooldown_minutes}
                onChange={(n) => patchStrategy({ strangle_stop_loss_cooldown_minutes: Math.max(0, Math.floor(n)) })}
