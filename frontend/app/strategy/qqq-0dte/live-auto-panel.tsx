...
type LiveWorkerConfig = {
  ...
  /** 无 expiry_date 时相对美东「交易日日期」的偏移天数：0=当日，1=次日到期 */
...
type StrategyRecommendationResponse = {
  ...
  option_quality?: Record<string, unknown>;
  disclaimer?: string;
  generated_at?: string;
...
function recommendationConfidenceClass(level: unknown): string {
  const v = String(level || "").toLowerCase();
  if (v === "high") return "border-emerald-400/40 bg-emerald-400/10 text-emerald-100";
  if (v === "medium") return "border-cyan-400/40 bg-cyan-400/10 text-cyan-100";
  return "border-amber-300/40 bg-amber-300/10 text-amber-100";
}

function strategyRecommendationFeature(rec: StrategyRecommendationResponse | null, key: string): unknown {
  const f = rec?.features;
  return f && typeof f === "object" ? f[key] : undefined;
}

function strategyRecommendationWindowText(rec: StrategyRecommendationResponse | null): string {
  const variant = String(rec?.recommended_variant || "");
  const windows = strategyRecommendationFeature(rec, "time_windows");
  if (!variant || !windows || typeof windows !== "object") return "";
  const w = asRecord((windows as Record<string, unknown>)[variant]);
  if (!w) return "";
  const state = String(w.state || "");
  const label = state === "open" ? "窗口内" : state === "before" ? "未到窗口" : state === "after" ? "已过窗口" : state || "未知";
  return `${label} ${String(w.now || "??:??")} ET · ${String(w.start || "??:??")}-${String(w.end || "??:??")}`;
}

function formatBeijingMinute(value: unknown): string {
...
function orderLifecycleStateLabel(value: unknown): string {
  const state = String(value || "").trim().toLowerCase();
  if (state === "submitted") return "已提交";
  if (state === "active") return "挂单中";
  if (state === "filled") return "已成交";
  if (state === "uncertain") return "状态不确定";
  if (state === "rejected") return "已拒绝";
  if (state === "cancelled") return "已取消";
  if (state === "cancel_requested") return "撤单中";
  if (state === "manual_review") return "人工复核";
  if (state === "unknown") return "未知";
  return state || "—";
}

function orderLifecycleEventLabel(value: unknown): string {
  const event = String(value || "").trim().toLowerCase();
  if (event === "entry_submitted") return "入场单提交";
  if (event === "entry_reconciliation") return "入场核对";
  if (event === "entry_active_orders_cancelled") return "未成交单撤单";
  if (event === "entry_submit_failed_after_order_ids") return "提交异常";
  if (event === "manual_review_lock") return "复核锁定";
  return event || "—";
}

function orderLifecycleToneClass(value: unknown): string {
  const state = String(value || "").trim().toLowerCase();
  if (state === "manual_review" || state === "rejected") return "text-rose-200";
  if (state === "uncertain" || state === "active" || state === "cancel_requested" || state === "submitted") return "text-amber-200";
  if (state === "filled" || state === "cancelled") return "text-emerald-300";
...
function qqq0dteStrategyDisplayName(strategy_config: Record<string, unknown> | undefined | null): string {
  ...
  if (v === "morning_double_strangle") return "早盘双宽跨";
...
function defaultLiveConfig(variant: QqqLiveWorkerVariant): LiveWorkerConfig {
  return {
    ...
    symbol: "QQQ.US",
    stock_options_mode: variant === "1dte",
    stock_pool: variant === "1dte" ? ["QQQ.US"] : [],
    history_days: 2,
    kline: "1m",
    poll_seconds: 30,
    dry_run: true,
...
function normalizeStockPoolText(value: unknown, fallbackSymbol: string): string {
  const fallback = (fallbackSymbol || "QQQ.US").trim().toUpperCase();
  if (Array.isArray(value)) {
    const out = value.map((x) => String(x || "").trim().toUpperCase()).filter(Boolean);
    return (out.length ? out : [fallback]).join(", ");
  }
  if (typeof value === "string" && value.trim()) return value;
  return fallback;
}

function parseStockPoolInput(value: string, fallbackSymbol: string): string[] {
  ...
  if (!out.length) out.push((fallbackSymbol || "QQQ.US").trim().toUpperCase());
  return out;
}
...
export function Qqq0dteLiveAutoPanel({
  ...
  const buildBodyForApi = useCallback((): LiveWorkerConfig | null => {
    try {
      ...
      const stockPool = liveVariant === "1dte" ? parseStockPoolInput(normalizeStockPoolText(draft.stock_pool, draft.symbol), draft.symbol) : draft.stock_pool;
      return { ...draft, stock_pool: stockPool, symbol: stockPool?.[0] || draft.symbol, strategy_config: filtered };
    } catch {
      setCfgErr("策略 JSON 解析失败");
      setJsonInlineErr("JSON 解析失败，请检查逗号、引号与括号是否匹配。");
      return null;
    }
  }, [draft, liveVariant, strategyJson]);
  ...
  const startWorker = async () => {
    ...
    const who = liveVariant === "1dte" ? "股票期权" : "QQQ 0DTE";
  ...
  const clearManualReviewLock = async () => {
    if (
      !confirm(
        "确认解除人工复核锁定？请先在券商后台核对已成交、未成交和已撤单订单，并确认当前仓位不会被 worker 误认为可继续开新仓。"
      )
    ) {
      return;
    }
    ...
    try {
      await apiPost(
        `${liveApiBase}/live-worker-manual-review/clear`,
        { note: "cleared_from_live_panel" },
        { timeoutMs: 15000, retries: 0 }
      );
      setCfgMsg("已解除人工复核锁定；worker 下一轮状态刷新后生效。");
      setTimeout(() => setCfgMsg(""), 5000);
      void refreshStatus();
    } catch (e: unknown) {
      setCfgErr(e instanceof Error ? e.message : String(e));
  ...
  const panelTitle = liveVariant === "1dte" ? "实盘自动交易（股票期权）" : "实盘自动交易（QQQ 0DTE）";
  ...
  const qIntradaySafety = asRecord(qRuntimeInner?.intraday_safety);
  const qIntradaySummary = asRecord(qRuntimeInner?.intraday_safety_summary);
  const qIntradaySafetyEnabled = qIntradaySummary?.enabled === true || qIntradaySafety?.enabled === true;
  const qIntradaySafetyText = qIntradaySafetyEnabled ? "已开启" : qIntradaySafety || qIntradaySummary ? "已关闭" : "未上报";
  const qIntradaySafetyClass = qIntradaySafetyEnabled ? "text-emerald-300" : "text-amber-300";
  const qIntradaySpreadPct = asNumber(qIntradaySummary?.max_bid_ask_spread_pct ?? qIntradaySafety?.max_bid_ask_spread_pct);
  const qIntradayLatestEntry = asString(qIntradaySummary?.latest_entry_hhmm_et ?? qIntradaySafety?.latest_entry_hhmm_et) || "—";
  const qIntradayLastReason = asString(qIntradaySummary?.last_reason) || "—";
  const qIntradayLastBlocked = qIntradaySummary?.last_blocked === true;
  const qIntradayUnmanagedCount = asNumber(qIntradaySummary?.unmanaged_positions_count) ?? 0;
  const qIntradayPostEntryActive = asNumber(qIntradaySummary?.post_entry_active_orders) ?? 0;
  const qIntradayForceCloseActive = asNumber(qIntradaySummary?.force_close_active_orders) ?? 0;
  const qIntradayManualReview = qIntradaySummary?.post_entry_manual_review_required === true;
  const qIntradayQualityBlocks = Array.isArray(qIntradaySummary?.quality_blocks) ? qIntradaySummary.quality_blocks.map(String) : [];
  const qReconExpected = asNumber(qIntradaySummary?.reconciliation_expected_contracts) ?? 0;
  const qReconFilled = asNumber(qIntradaySummary?.reconciliation_filled_contracts) ?? 0;
  const qReconUncertain = qIntradaySummary?.reconciliation_uncertain === true;
  const qReconAllFilled = qIntradaySummary?.reconciliation_all_filled === true;
  const qLifecycle = asRecord(qIntradaySummary?.order_lifecycle);
  const qLifecycleLatest = asRecord(qLifecycle?.latest);
  const qLifecycleState = asString(qLifecycle?.latest_state) || asString(qLifecycleLatest?.state) || "—";
  const qLifecycleEvent = asString(qLifecycle?.latest_event) || asString(qLifecycleLatest?.event) || "—";
  const qLifecycleAt = asString(qLifecycle?.latest_at) || asString(qLifecycleLatest?.at) || "—";
  const qLifecycleNeedsAttention = qLifecycle?.needs_attention === true;
  const qLifecyclePendingOrders = Array.isArray(qLifecycle?.pending_order_ids)
    ? qLifecycle.pending_order_ids.map(String).filter(Boolean)
    : [];
  const qLifecycleRecent = Array.isArray(qLifecycle?.recent)
    ? qLifecycle.recent.map(asRecord).filter((x): x is Record<string, unknown> => Boolean(x)).slice(-3).reverse()
    : [];
  const qLifecycleOrders = Array.isArray(qLifecycleLatest?.order_ids)
    ? qLifecycleLatest.order_ids.map(String).filter(Boolean)
    : [];
  const qManualReviewLocked = qIntradaySummary?.manual_review_locked === true;
  const qManualReviewReason = asString(qIntradaySummary?.manual_review_reason) || "—";
  const qManualReviewCreatedAt = asString(qIntradaySummary?.manual_review_created_at) || "—";
  const qManualReviewOrderIds = Array.isArray(qIntradaySummary?.manual_review_order_ids)
    ? qIntradaySummary.manual_review_order_ids.map(String).filter(Boolean)
    : [];
  const qManualReviewSymbols = Array.isArray(qIntradaySummary?.manual_review_expected_symbols)
    ? qIntradaySummary.manual_review_expected_symbols.map(String).filter(Boolean)
    : [];
  const qLastActionText = asString(qLastAction?.action) || "—";
  const recCurrentVariant = asString(strategyRecommendationFeature(strategyRec, "current_strategy_variant"));
  const recMatchesCurrent = strategyRecommendationFeature(strategyRec, "recommended_matches_current_strategy") === true;
  const recSameTimeVol = asNumber(strategyRecommendationFeature(strategyRec, "same_time_volume_ratio_today_vs_recent_days"));
  const recWindowText = strategyRecommendationWindowText(strategyRec);
  const recWarnings = Array.isArray(strategyRec?.warnings) ? strategyRec.warnings : [];
  const recOptionQuality = asRecord(strategyRec?.option_quality);
  const recOptionQualityStatus = asString(recOptionQuality?.status) || "not_checked";
  const recOptionQualityNote = asString(recOptionQuality?.note);
  const recConfidenceLevel = asString(strategyRec?.confidence?.level) || "low";
  const recConfidenceLabel = asString(strategyRec?.confidence?.label) || (recConfidenceLevel === "high" ? "高" : recConfidenceLevel === "medium" ? "中" : "低");
  ...
  return (
    <div className="panel space-y-4 border-emerald-500/25 bg-gradient-to-br from-slate-900/95 via-slate-900/95 to-emerald-950/20">
      ...
      {liveVariant === "1dte" ? (
        <div className="rounded-lg border border-cyan-400/25 bg-cyan-950/10 p-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div>
              <div className="text-sm font-semibold text-cyan-100">日内实盘保护</div>
              <div className="mt-1 text-[11px] text-slate-500">只影响股票期权日内 worker；不会触发下单。</div>
            </div>
            <span className={`rounded-full border px-2 py-0.5 text-xs ${qIntradaySafetyEnabled ? "border-emerald-400/35 bg-emerald-400/10" : "border-amber-300/35 bg-amber-300/10"} ${qIntradaySafetyClass}`}>
              {qIntradaySafetyText}
            </span>
          </div>
          <div className="mt-3 grid grid-cols-1 gap-2 md:grid-cols-3 xl:grid-cols-6">
            <div className="rounded-md border border-slate-700 bg-slate-950/35 px-3 py-2">
              <div className="text-[11px] text-slate-500">期权链门槛</div>
              <div className="mt-1 text-sm text-slate-200">
                价差 ≤ {qIntradaySpreadPct == null ? "—" : `${(qIntradaySpreadPct * 100).toFixed(1)}%`}
              </div>
              <div className="mt-1 text-[10px] text-slate-500">最晚开仓 {qIntradayLatestEntry} ET</div>
            </div>
            <div className="rounded-md border border-slate-700 bg-slate-950/35 px-3 py-2">
              <div className="text-[11px] text-slate-500">最近拦截</div>
              <div className={`mt-1 text-sm ${qIntradayLastBlocked ? "text-amber-200" : "text-slate-300"}`}>
                {qIntradayLastBlocked ? "已拦截" : "无拦截"}
              </div>
              <div className="mt-1 break-all font-mono text-[10px] text-slate-500">{qIntradayLastReason}</div>
            </div>
            <div className="rounded-md border border-slate-700 bg-slate-950/35 px-3 py-2">
              <div className="text-[11px] text-slate-500">托管仓位</div>
              <div className={`mt-1 text-sm ${qIntradayUnmanagedCount > 0 ? "text-amber-200" : "text-emerald-300"}`}>
                未托管 {qIntradayUnmanagedCount}
              </div>
              <div className="mt-1 text-[10px] text-slate-500">检测到未托管同标的期权会阻断新开仓</div>
            </div>
            <div className="rounded-md border border-slate-700 bg-slate-950/35 px-3 py-2">
              <div className="text-[11px] text-slate-500">订单执行</div>
              <div className={`mt-1 text-sm ${qIntradayManualReview || qIntradayPostEntryActive > 0 ? "text-amber-200" : "text-slate-300"}`}>
                active {qIntradayPostEntryActive} · 强平撤单 {qIntradayForceCloseActive}
              </div>
              <div className="mt-1 text-[10px] text-slate-500">{qIntradayManualReview ? "需要人工复核" : "未发现未完成入场单"}</div>
            </div>
            <div className="rounded-md border border-slate-700 bg-slate-950/35 px-3 py-2">
              <div className="text-[11px] text-slate-500">成交回执</div>
              <div className={`mt-1 text-sm ${qReconUncertain ? "text-amber-200" : qReconAllFilled ? "text-emerald-300" : "text-slate-300"}`}>
                {qReconExpected > 0 ? `${qReconFilled}/${qReconExpected} 张` : "未上报"}
              </div>
              <div className="mt-1 text-[10px] text-slate-500">
                {qReconUncertain ? "状态不确定，已触发复核保护" : qReconAllFilled ? "订单与持仓已核对" : "等待下一次入场回执"}
              </div>
            </div>
            <div className="rounded-md border border-slate-700 bg-slate-950/35 px-3 py-2">
              <div className="text-[11px] text-slate-500">订单生命周期</div>
              <div className={`mt-1 text-sm ${orderLifecycleToneClass(qLifecycleState)}`}>
                {orderLifecycleStateLabel(qLifecycleState)}
              </div>
              <div className="mt-1 break-all font-mono text-[10px] text-slate-500">
                {orderLifecycleEventLabel(qLifecycleEvent)} · {formatBeijingMinute(qLifecycleAt) || qLifecycleAt}
              </div>
              <div className={`mt-1 text-[10px] ${qLifecycleNeedsAttention ? "text-amber-200" : "text-slate-500"}`}>
                {qLifecycleNeedsAttention ? "需要检查订单状态" : "无待处理生命周期事件"}
              </div>
              {(qLifecyclePendingOrders.length > 0 ? qLifecyclePendingOrders : qLifecycleOrders).length > 0 ? (
                <div
                  className="mt-1 truncate font-mono text-[10px] text-slate-600"
                  title={(qLifecyclePendingOrders.length > 0 ? qLifecyclePendingOrders : qLifecycleOrders).join(", ")}
                >
                  {(qLifecyclePendingOrders.length > 0 ? qLifecyclePendingOrders : qLifecycleOrders).join(", ")}
                </div>
              ) : null}
            </div>
          </div>
          {qLifecycleRecent.length > 0 ? (
            <div className="mt-2 rounded-md border border-slate-700/80 bg-slate-950/25 px-3 py-2">
              <div className="mb-1 text-[11px] font-semibold text-slate-400">最近订单事件</div>
              <div className="grid grid-cols-1 gap-1 md:grid-cols-3">
                {qLifecycleRecent.map((event, idx) => {
                  const state = asString(event.state);
                  const orderIds = Array.isArray(event.order_ids) ? event.order_ids.map(String).filter(Boolean) : [];
                  const reason = asString(event.reason);
                  return (
                    <div key={`${asString(event.at)}-${idx}`} className="rounded border border-slate-800 bg-slate-950/35 px-2 py-1.5">
                      <div className={`text-xs font-semibold ${orderLifecycleToneClass(state)}`}>{orderLifecycleStateLabel(state)}</div>
                      <div className="mt-0.5 text-[10px] text-slate-500">
                        {orderLifecycleEventLabel(event.event)} · {formatBeijingMinute(event.at) || asString(event.at) || "—"}
                      </div>
                      <div className="mt-0.5 truncate font-mono text-[10px] text-slate-600" title={orderIds.join(", ") || reason}>
                        {orderIds.length > 0 ? orderIds.join(", ") : reason || "—"}
                      </div>
                    </div>
                  );
                })}
          ...
          {qIntradayQualityBlocks.length > 0 ? (
            <div className="mt-2 rounded-md border border-amber-300/25 bg-amber-300/10 px-3 py-2 text-xs text-amber-100">
              期权链质量阻断：{qIntradayQualityBlocks.join(" · ")}
            </div>
          ) : null}
          <div
            className={`mt-2 rounded-md border px-3 py-2 ${
              qManualReviewLocked ? "border-rose-400/35 bg-rose-950/25" : "border-emerald-400/25 bg-emerald-950/15"
            }`}
          >
            {qManualReviewLocked ? (
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <div className="text-sm font-semibold text-rose-100">人工复核锁定</div>
                  <div className="mt-1 text-xs leading-relaxed text-rose-100/80">
                    worker 已暂停新的日内期权开仓。请先在券商后台核对成交、撤单和当前持仓，再决定是否解除。
                  </div>
                  <div className="mt-2 break-all font-mono text-[10px] text-rose-100/70">
                    reason={qManualReviewReason} · created={qManualReviewCreatedAt}
                  </div>
                  {qManualReviewOrderIds.length > 0 ? (
                    <div className="mt-1 break-all font-mono text-[10px] text-rose-100/70">orders={qManualReviewOrderIds.join(", ")}</div>
                  ) : null}
                  {qManualReviewSymbols.length > 0 ? (
                    <div className="mt-1 break-all font-mono text-[10px] text-rose-100/70">symbols={qManualReviewSymbols.join(", ")}</div>
                  ) : null}
                </div>
                <button
                  type="button"
                  className="rounded-md border border-rose-300/35 bg-rose-950/40 px-3 py-1.5 text-xs font-medium text-rose-50 hover:bg-rose-900/50 disabled:cursor-not-allowed disabled:opacity-50"
                  disabled={opLoading || !canRunOptionAuto}
                  onClick={() => void clearManualReviewLock()}
                >
                  确认已复核，解除锁定
                </button>
              </div>
              ...
              <div>
                <div className="text-sm font-semibold text-emerald-100">人工复核正常</div>
                <div className="mt-1 text-xs leading-relaxed text-emerald-100/75">
                  当前没有复核锁定；仅当检测到未托管持仓、入场成交状态不确定、持仓无法核对等特殊场景时才会转为红色锁定。
                </div>
              </div>
            )}
          </div>
      ...
      <div className="rounded-lg border border-amber-500/30 bg-amber-950/15 p-3">
        ...
        <p className="mt-1 text-[11px] leading-snug text-slate-500">
          ...
          依据包括现价、相对前收涨跌、前交易日高低、同时间段量能比、VIX 和当前配置阈值等；<strong>不参与下单</strong>，与下方 strategy_variant 无关。Worker 失败时可查看{" "}
        ...
        {strategyRec?.ok === false ? (
          ...
          <div className="mt-2 space-y-2 text-xs">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className={`rounded-full border px-2 py-0.5 text-[10px] ${recommendationConfidenceClass(recConfidenceLevel)}`}>
                置信度 {recConfidenceLabel}
              </span>
              {recCurrentVariant ? (
                <span
                  className={`rounded-full border px-2 py-0.5 text-[10px] ${
                    recMatchesCurrent ? "border-emerald-400/35 bg-emerald-400/10 text-emerald-100" : "border-amber-300/35 bg-amber-300/10 text-amber-100"
                  }`}
                >
                  {recMatchesCurrent ? "与当前实盘策略一致" : `当前实盘：${qqq0dteStrategyDisplayName({ strategy_variant: recCurrentVariant })}`}
                </span>
              ) : null}
            </div>
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
              <div className="rounded-md border border-amber-200/10 bg-slate-950/35 px-2 py-1.5">
                <div className="text-[10px] text-slate-500">适用窗口</div>
                <div className="mt-0.5 text-slate-200">{recWindowText || "—"}</div>
              </div>
              <div className="rounded-md border border-amber-200/10 bg-slate-950/35 px-2 py-1.5">
                <div className="text-[10px] text-slate-500">同时间段量能</div>
                <div className="mt-0.5 text-slate-200">{recSameTimeVol == null ? "—" : `${recSameTimeVol.toFixed(2)}×`}</div>
              </div>
              <div className="rounded-md border border-amber-200/10 bg-slate-950/35 px-2 py-1.5">
                <div className="text-[10px] text-slate-500">期权链检查</div>
                <div className="mt-0.5 text-slate-200">{recOptionQualityStatus === "not_checked" ? "未检查，下单前再校验" : recOptionQualityStatus}</div>
              </div>
            </div>
            {strategyRec.generated_at ? (
              <p className="font-mono text-[10px] text-slate-500">生成时间（北京时间）：{formatBeijingMinute(strategyRec.generated_at)}</p>
            ) : null}
            {strategyRec.source ? (
            ...
            {recWarnings.length > 0 ? (
              <div className="rounded-md border border-amber-300/25 bg-amber-300/10 px-2 py-1.5 text-amber-100">
                <div className="text-[10px] font-semibold">注意事项</div>
            ...
            {recOptionQualityNote ? <p className="text-[10px] leading-snug text-slate-500">{recOptionQualityNote}</p> : null}
            ...
            onChange={(e) =>
              setDraft((d) => {
                ...
                return {
                  ...d,
                  symbol,
                  stock_pool: liveVariant === "1dte" ? parseStockPoolInput(normalizeStockPoolText(d.stock_pool, symbol), symbol) : d.stock_pool,
                };
              })
            }
          />
          ...
          <>
            <label className="flex cursor-pointer items-center gap-2 pt-7">
              <input
                ...
                checked={draft.stock_options_mode !== false}
                onChange={(e) => setDraft((d) => ({ ...d, stock_options_mode: e.target.checked }))}
              />
              <span className="text-sm text-slate-300">股票池模式</span>
            </label>
            <label className="space-y-1 md:col-span-2 lg:col-span-3">
              <div className="field-label">股票池（逗号或换行分隔）</div>
              <textarea
                className="input-base min-h-20 w-full font-mono text-sm"
                value={normalizeStockPoolText(draft.stock_pool, draft.symbol)}
                onChange={(e) => {
                  const pool = parseStockPoolInput(e.target.value, draft.symbol);
                  setDraft((d) => ({ ...d, stock_pool: pool, symbol: pool[0] || d.symbol }));
                }}
                placeholder="QQQ.US, NVDA.US, TSLA.US"
              />
              <p className="text-[11px] leading-snug text-slate-500">
                Worker 会逐个拉取股票池标的的分时线和期权链，每个标的独立生成信号、独立保存恢复快照；默认保留 QQQ.US。
              </p>
            </label>
          </>
        ) : null}
              ...
              <>
                <label className="block space-y-1">
                  ...
                  >
                    ...
                    <option value="morning_double_strangle">早盘双宽跨 morning_double_strangle</option>
                ...
                {strFromSc(strategyConfigParsed, "strategy_variant", "reaction_zone") === "morning_double_strangle" ? (
                  <div className="rounded-md border border-indigo-500/20 bg-indigo-950/15 p-2 space-y-2">
                    <div className="text-[11px] font-medium text-indigo-200/80">早盘双宽跨</div>
                    <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                      <label className="space-y-1">
                        <span className="text-[11px] text-slate-500">开仓开始（美东 HH:MM）strangle_entry_start_hhmm_et</span>
                        <input className="input-base w-full font-mono text-xs" value={strFromSc(strategyConfigParsed, "strangle_entry_start_hhmm_et", "09:35")} onChange={(e) => patchStrategyConfig({ strangle_entry_start_hhmm_et: e.target.value })} />
                      </label>
                      <label className="space-y-1">
                        <span className="text-[11px] text-slate-500">开仓结束 strangle_entry_end_hhmm_et</span>
                        <input className="input-base w-full font-mono text-xs" value={strFromSc(strategyConfigParsed, "strangle_entry_end_hhmm_et", "10:00")} onChange={(e) => patchStrategyConfig({ strangle_entry_end_hhmm_et: e.target.value })} />
                      </label>
                      <label className="space-y-1">
                        <span className="text-[11px] text-slate-500">强制平仓 strangle_force_close_hhmm_et</span>
                        <input className="input-base w-full font-mono text-xs" value={strFromSc(strategyConfigParsed, "strangle_force_close_hhmm_et", "12:00")} onChange={(e) => patchStrategyConfig({ strangle_force_close_hhmm_et: e.target.value })} />
                      </label>
                      <label className="space-y-1">
                        <span className="text-[11px] text-slate-500">相对前收最大偏离（%）strangle_range_pct×100</span>
                        <input className="input-base w-full" type="number" step={0.01} min={0} value={numFromSc(strategyConfigParsed, "strangle_range_pct", 0.003) * 100} onChange={(e) => patchStrategyConfig({ strangle_range_pct: Math.max(0, (Number(e.target.value) || 0) / 100) })} />
                      </label>
                      <label className="space-y-1">
                        <span className="text-[11px] text-slate-500">Call 长腿 OTM double_strangle_call_long_strikes_otm</span>
                        <input className="input-base w-full" type="number" step={1} min={1} value={Math.max(1, Math.floor(numFromSc(strategyConfigParsed, "double_strangle_call_long_strikes_otm", 2)))} onChange={(e) => patchStrategyConfig({ double_strangle_call_long_strikes_otm: Math.max(1, Math.floor(Number(e.target.value) || 2)) })} />
                      </label>
                      <label className="space-y-1">
                        <span className="text-[11px] text-slate-500">Call 短腿 OTM double_strangle_call_short_strikes_otm</span>
                        <input className="input-base w-full" type="number" step={1} min={0} value={Math.max(0, Math.floor(numFromSc(strategyConfigParsed, "double_strangle_call_short_strikes_otm", 1)))} onChange={(e) => patchStrategyConfig({ double_strangle_call_short_strikes_otm: Math.max(0, Math.floor(Number(e.target.value) || 1)) })} />
                      </label>
                      <label className="space-y-1">
                        <span className="text-[11px] text-slate-500">Put 长腿 OTM double_strangle_put_long_strikes_otm</span>
                        <input className="input-base w-full" type="number" step={1} min={1} value={Math.max(1, Math.floor(numFromSc(strategyConfigParsed, "double_strangle_put_long_strikes_otm", 2)))} onChange={(e) => patchStrategyConfig({ double_strangle_put_long_strikes_otm: Math.max(1, Math.floor(Number(e.target.value) || 2)) })} />
                      </label>
                      <label className="space-y-1">
                        <span className="text-[11px] text-slate-500">Put 短腿 OTM double_strangle_put_short_strikes_otm</span>
                        <input className="input-base w-full" type="number" step={1} min={0} value={Math.max(0, Math.floor(numFromSc(strategyConfigParsed, "double_strangle_put_short_strikes_otm", 1)))} onChange={(e) => patchStrategyConfig({ double_strangle_put_short_strikes_otm: Math.max(0, Math.floor(Number(e.target.value) || 1)) })} />
                      </label>
                      <label className="space-y-1">
                        <span className="text-[11px] text-slate-500">Call 长腿止盈（%）</span>
                        <input className="input-base w-full" type="number" step={1} min={0} value={numFromSc(strategyConfigParsed, "double_strangle_call_long_leg_take_profit_pct", 1) * 100} onChange={(e) => patchStrategyConfig({ double_strangle_call_long_leg_take_profit_pct: Math.max(0, (Number(e.target.value) || 0) / 100) })} />
                      </label>
                      <label className="space-y-1">
                        <span className="text-[11px] text-slate-500">Call 短腿止盈（%）</span>
                        <input className="input-base w-full" type="number" step={1} min={0} value={numFromSc(strategyConfigParsed, "double_strangle_call_short_leg_take_profit_pct", 0.35) * 100} onChange={(e) => patchStrategyConfig({ double_strangle_call_short_leg_take_profit_pct: Math.max(0, (Number(e.target.value) || 0) / 100) })} />
                      </label>
                      <label className="space-y-1">
                        <span className="text-[11px] text-slate-500">Put 长腿止盈（%）</span>
                        <input className="input-base w-full" type="number" step={1} min={0} value={numFromSc(strategyConfigParsed, "double_strangle_put_long_leg_take_profit_pct", 1) * 100} onChange={(e) => patchStrategyConfig({ double_strangle_put_long_leg_take_profit_pct: Math.max(0, (Number(e.target.value) || 0) / 100) })} />
                      </label>
                      <label className="space-y-1">
                        <span className="text-[11px] text-slate-500">Put 短腿止盈（%）</span>
                        <input className="input-base w-full" type="number" step={1} min={0} value={numFromSc(strategyConfigParsed, "double_strangle_put_short_leg_take_profit_pct", 0.35) * 100} onChange={(e) => patchStrategyConfig({ double_strangle_put_short_leg_take_profit_pct: Math.max(0, (Number(e.target.value) || 0) / 100) })} />
                      </label>
                      <label className="space-y-1">
                        <span className="text-[11px] text-slate-500">单腿止损（%）double_strangle_single_leg_stop_loss_pct×100</span>
                        <input className="input-base w-full" type="number" step={1} min={0} value={numFromSc(strategyConfigParsed, "double_strangle_single_leg_stop_loss_pct", 0.35) * 100} onChange={(e) => patchStrategyConfig({ double_strangle_single_leg_stop_loss_pct: Math.max(0, (Number(e.target.value) || 0) / 100) })} />
                      </label>
                      <label className="space-y-1">
                        <span className="text-[11px] text-slate-500">四腿组合止盈（%）double_strangle_combo_take_profit_pct×100</span>
                        <input className="input-base w-full" type="number" step={1} min={0} value={numFromSc(strategyConfigParsed, "double_strangle_combo_take_profit_pct", 0.6) * 100} onChange={(e) => patchStrategyConfig({ double_strangle_combo_take_profit_pct: Math.max(0, (Number(e.target.value) || 0) / 100) })} />
                      </label>
                      <label className="space-y-1">
                        <span className="text-[11px] text-slate-500">四腿组合止损（%）double_strangle_combo_stop_loss_pct×100</span>
                        <input className="input-base w-full" type="number" step={1} min={0} value={numFromSc(strategyConfigParsed, "double_strangle_combo_stop_loss_pct", 0.3) * 100} onChange={(e) => patchStrategyConfig({ double_strangle_combo_stop_loss_pct: Math.max(0, (Number(e.target.value) || 0) / 100) })} />
                      </label>
                      <label className="space-y-1">
                        <span className="text-[11px] text-slate-500">四腿总权利金上限 double_strangle_max_total_debit，0=不限制</span>
                        <input className="input-base w-full" type="number" step={0.01} min={0} value={numFromSc(strategyConfigParsed, "double_strangle_max_total_debit", 0)} onChange={(e) => patchStrategyConfig({ double_strangle_max_total_debit: Math.max(0, Number(e.target.value) || 0) })} />
                      </label>
                      <label className="space-y-1">
                        <span className="text-[11px] text-slate-500">回测标的价字段 strangle_underlying_field</span>
                        <select className="input-base w-full text-sm" value={strFromSc(strategyConfigParsed, "strangle_underlying_field", "low")} onChange={(e) => patchStrategyConfig({ strangle_underlying_field: e.target.value })}>
                          <option value="open">open</option>
                          <option value="high">high</option>
                          <option value="low">low</option>
                          <option value="close">close</option>
