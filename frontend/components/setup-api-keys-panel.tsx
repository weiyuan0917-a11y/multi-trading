...
export function SetupApiKeysPanel() {
  ...
  const [applyBusy, setApplyBusy] = useState<"" | "0dte" | "1dte" | "swing" | "at">("");
  ...
  const applyToStockOptionsSwing = async () => {
    const k = lastPlainKey.trim();
    if (!k) {
      setErr("请先在上方「创建新 Key」并保留本页展示的明文，再写入配置。");
      return;
    ...
    setErr("");
    try {
      await apiPut("/strategy/stock-options-swing/live-worker-config", { api_key: k }, { timeoutMs: 20000, retries: 0 });
      setErr("");
      window.alert(
        "已写入 data/stock_options_swing/live_worker_config.json 的 api_key。若股票期权中长线 Worker 已在运行，请重启 Worker 以加载新密钥。"
      );
    } catch (e) {
      setErr(e instanceof Error ? e.message : String(e));
    } finally {
      ...
      {lastPlainKey ? (
        <div className="rounded border border-amber-500/50 bg-amber-950/25 p-3 space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              {applyBusy === "1dte" ? "写入中…" : "一键写入股票期权日内实盘配置"}
            </button>
            <button
              type="button"
              className="btn-secondary text-xs"
              disabled={applyBusy === "swing"}
              onClick={() => void applyToStockOptionsSwing()}
            >
              {applyBusy === "swing" ? "写入中…" : "一键写入股票期权中长线实盘配置"}
            </button>
            <button
              type="button"
              className="btn-secondary text-xs"
