...
function parseSubscriptionExpiresAt(license: LocalLicense): number | null {
  const rawMs = Number(license.subscription_current_period_end || license.subscriptionCurrentPeriodEnd || 0);
  if (Number.isFinite(rawMs) && rawMs > 0) {
    const ms = rawMs < 100000000000 ? rawMs * 1000 : rawMs;
    return ms;
  }
  const raw = String(license.subscription_expires_at || license.subscriptionExpiresAt || "").trim();
  if (!raw) return null;
  const ts = Date.parse(raw);
  return Number.isFinite(ts) ? ts : null;
}
...
export function isLocalLicenseUsable(
  ...
  if (license.entitlement_valid === true || license.subscription_valid === true) return true;
  const subscriptionExpiresAt = parseSubscriptionExpiresAt(license);
  if (subscriptionExpiresAt !== null && subscriptionExpiresAt > Date.now()) return true;
