...
Until cloud billing is wired, use:

```env
NEXT_PUBLIC_MT_PLAN=free
```

Local dev can temporarily set:

```env
NEXT_PUBLIC_MT_PLAN=premium
```

For real cloud billing, plan and role should come from cloud auth/session data, not from local storage or public env overrides.

## Local License Cache

The Local Agent now has a signed local license cache skeleton for China-friendly startup:
...
By default unsigned license imports are rejected:
...
For the MVP HMAC flow, set the same secret in Convex as `CONVEX_LOCAL_LICENSE_SIGNING_SECRET` and in the Local Agent as `LOCAL_LICENSE_SIGNING_SECRET`. For production, replace this with asymmetric signing: the cloud billing system signs with a private key, and the Local Agent verifies with a public key. Unsigned import should only be enabled on a trusted development machine.
...
4. The page creates a pending order and shows WeChat / Alipay / Wise QR payment instructions.
5. Customer pays with the order number as payment remark.
6. Admin opens `/admin/orders`, verifies the payment in WeChat / Alipay / Wise, then clicks "confirm payment and issue License".
7. Convex signs the License and sends it via Resend email.

Feishu assisted review is also supported:

1. Customer creates a payment order on `/billing`.
2. The Next.js billing route sends a Feishu notification with the order number, owner_id, plan, amount, payment method, and confirmation command.
3. Admin verifies the transfer outside the app.
4. Admin replies in Feishu with `确认收款 <订单号> <流水号或备注>` or `确认收款并发证 <订单号> <流水号或备注>`.
5. The Feishu command bot calls the same Convex admin action used by `/admin/orders`, so the order is marked paid, the License is signed, and the existing Resend email delivery path is used.

Feishu billing notification variables:

```env
FEISHU_BILLING_WEBHOOK_URL=
FEISHU_BILLING_WEBHOOK_SECRET=
FEISHU_BILLING_APP_ID=
FEISHU_BILLING_APP_SECRET=
FEISHU_BILLING_CHAT_ID=
FEISHU_BILLING_ADMIN_OPEN_IDS=
FEISHU_BILLING_ADMIN_CHAT_IDS=
```

If these are empty in local development, the Next.js route falls back to `mcp_server/notification_config.json` and uses the first configured `feishu_bots` webhook.
If the custom webhook returns an error such as `Bot Not Enabled`, the route falls back to the Feishu app channel (`feishu_app.app_id`, `feishu_app.app_secret`, and `feishu_app.scheduled_chat_id` in `mcp_server/notification_config.json`, or the `FEISHU_BILLING_APP_*` variables above).

`FEISHU_BILLING_ADMIN_OPEN_IDS` and `FEISHU_BILLING_ADMIN_CHAT_IDS` are optional comma-separated allowlists for Feishu payment commands. If either allowlist is set, only matching users or chats can run `付款订单` / `确认收款` commands. Keep them empty only for trusted local development.

The Feishu command bot needs access to the same billing environment values:
...
For local development it also reads `frontend/.env.local`, so the command bot can confirm orders without duplicating those two values in the root `.env`.
...
NEXT_PUBLIC_PAYMENT_QR_WISE_URL=
```
...
frontend/public/payments/wechat-qr.jpg
frontend/public/payments/alipay-qr.jpg
frontend/public/payments/wise-qr.jpg
```
