...
export default function AdminOrdersPage() {
  ...
  const runAction = async (row: ManualOrder, action: "confirm" | "cancel") => {
    if (action === "confirm") {
      const ok = window.confirm(`确认 ${row.orderNo} 已收到 ¥${row.amountCny}，并给 ${row.ownerId} 签发 ${PLAN_LABELS[row.plan]} License？`);
      if (!ok) return;
    }
    if (action === "cancel") {
              ...
              {rows.map((row) => {
                ...
                return (
                  <tr key={row.id} className="border-t border-slate-700/70">
                    ...
                    <td className="px-4 py-3">
                      <div className="font-semibold text-slate-100">¥{row.amountCny.toLocaleString("zh-CN")}</div>
