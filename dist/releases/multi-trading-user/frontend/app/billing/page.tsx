...
const BILLING_ORDER_API_URL = process.env.NEXT_PUBLIC_BILLING_ORDER_API_URL || "";

function priceLabel(plan: Plan, cycle: BillingCycle) {
  return `¥${PRICES[plan][cycle].toLocaleString("zh-CN")}`;
}
...
export default function BillingPage() {
  ...
  const canCreateLocalOrder = ADMIN_EDITION_ENABLED || Boolean(BILLING_ORDER_API_URL);
  const canSubmit = useMemo(() => {
    return Boolean(canCreateLocalOrder && email.trim().includes("@") && /^[a-z0-9][a-z0-9_-]{2,39}$/.test(ownerId.trim().toLowerCase()));
  }, [canCreateLocalOrder, email, ownerId]);
  ...
  const createOrder = async () => {
    if (!canCreateLocalOrder) {
      ...
      setError("用户版不在本机创建收款订单。请配置 NEXT_PUBLIC_BILLING_PORTAL_URL 指向云端购买入口。");
      return;
    }
    if (!canSubmit) {
    ...
    setMessage("");
    try {
      const response = await fetch(BILLING_ORDER_API_URL || "/api/billing/manual-orders", {
        method: "POST",
      ...
      <div className="panel border-cyan-500/20 bg-gradient-to-br from-slate-900/95 via-slate-900/95 to-cyan-950/20">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">
              {canCreateLocalOrder
                ...
                : "用户版不在本机保存收款后台和签发能力。请通过云端购买入口完成订购，付款后导入邮件中的本地 License。"}
            </p>
          </div>
          <div className="rounded-2xl border border-slate-700 bg-slate-950/40 px-4 py-3 text-right">
            ...
            <div className="mt-1 text-3xl font-bold text-cyan-100">¥{amount.toLocaleString("zh-CN")}</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(360px,0.62fr)]">
        <section className="panel space-y-5">
          ...
          <button
            ...
            disabled={loading || (canCreateLocalOrder && !canSubmit)}
            onClick={() => void createOrder()}
          >
            {loading ? "正在生成订单..." : canCreateLocalOrder ? "生成付款订单" : BILLING_PORTAL_URL ? "前往云端购买" : "等待配置购买入口"}
          </button>
        </section>
          ...
          <div>
            ...
            <div className="mt-3 rounded-2xl border border-slate-700 bg-slate-950/35 p-4">
              ...
              <div className="flex items-center justify-between gap-4 border-b border-slate-700/70 py-3">
                ...
                <span className="text-xl font-bold text-cyan-100">¥{amount.toLocaleString("zh-CN")}</span>
              </div>
              <div className="flex items-center justify-between gap-4 border-b border-slate-700/70 py-3">
