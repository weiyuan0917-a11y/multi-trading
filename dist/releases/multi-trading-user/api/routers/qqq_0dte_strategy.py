...
def qqq_0dte_live_worker_config_get_route() -> dict[str, Any]:
    """读取 `data/qqq_0dte/live_worker_config.json`（与实盘 Worker 共用），缺省字段由后端补齐。"""
    return {"config": rt.qqq_0dte_live_worker_config_get()}
...
def qqq_0dte_live_worker_config_put_route(
    ...
    return rt.qqq_0dte_live_worker_config_put(body if isinstance(body, dict) else {})


@router.get("/strategy/qqq-0dte/live-worker-decision-tail")
def qqq_0dte_live_worker_decision_tail_route(limit: int = Query(20, ge=1, le=100)) -> dict[str, Any]:
    """实盘 Worker 每根 K 线决策摘要（JSONL 尾部）；用于排查未下单原因。"""
    return rt.qqq_0dte_live_worker_decision_tail_get(limit)
...
def qqq_1dte_live_worker_config_get_route() -> dict[str, Any]:
    return {"config": rt.qqq_1dte_live_worker_config_get()}
...
def qqq_1dte_live_worker_config_put_route(
    ...
    require_entitlement(authorization, x_local_owner, "option_auto_trading", x_api_key)
    return rt.qqq_1dte_live_worker_config_put(body if isinstance(body, dict) else {})


@router.get("/strategy/qqq-1dte/live-worker-decision-tail")
def qqq_1dte_live_worker_decision_tail_route(limit: int = Query(20, ge=1, le=100)) -> dict[str, Any]:
    return rt.qqq_1dte_live_worker_decision_tail_get(limit)
