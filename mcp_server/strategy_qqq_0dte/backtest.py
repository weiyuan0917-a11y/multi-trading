...
def _sum_open_premium_debit_usd(trades: list[dict[str, Any]], contract_multiplier: int) -> float:
    ...
    for t in trades:
        ...
        if side == "double_strangle":
            legs = t.get("double_strangle_legs")
            if isinstance(legs, dict):
                total += sum(float(x.get("entry_px") or 0.0) for x in legs.values() if isinstance(x, dict)) * qty * mult
        elif side == "strangle":
...
def run_qqq_0dte_backtest(bars: Sequence[Bar], cfg: Qqq0dteConfig | None = None) -> dict[str, Any]:
    ...
    for i, _bar in enumerate(sorted_bars):
        ...
        if r.entry_snapshot:
            ...
            if str(es.get("side") or "") == "double_strangle":
                legs_map = es.get("double_strangle_legs")
                fee_legs = []
                if isinstance(legs_map, dict):
                    for leg in legs_map.values():
                        if isinstance(leg, dict):
                            fee_legs.append(
                                {
                                    "side": "buy",
                                    "contracts": qty,
                                    "price": float(leg.get("entry_px") or 0.0),
                                    "symbol": "SYNTH.OPT",
                                }
                            )
                fee_buy = float(estimate_us_option_multi_leg_fee(fee_legs)["total_fee"]) if fee_legs else 0.0
            elif str(es.get("side") or "") == "strangle":
        ...
        if r.close_position and r.exit_snapshot:
            ...
            if str(xs.get("side") or "") == "double_strangle":
                legs_map = xs.get("double_strangle_legs")
                close_keys = xs.get("close_leg_keys")
                leg_exit_px = xs.get("leg_exit_px")
                ...
                if isinstance(legs_map, dict):
                    for key in close_keys:
                        leg = legs_map.get(str(key))
                        if not isinstance(leg, dict):
                            continue
                        entry = float(leg.get("entry_px") or 0.0)
                        xp = float(leg_exit_px.get(str(key)) or 0.0)
                        if entry <= 0:
                            continue
                        gross += (xp - entry) * qty * mult
                        fee_legs.append({"side": "sell", "contracts": qty, "price": max(0.0, xp), "symbol": "SYNTH.OPT"})
                fee_sell = float(estimate_us_option_multi_leg_fee(fee_legs)["total_fee"]) if fee_legs else 0.0
            elif str(xs.get("side") or "") == "strangle":
            ...
            elif str(xs.get("side") or "") == "double_strangle":
                partial_leg = str(xs.get("strangle_partial_leg") or "none")
                pos = ctl._pos
                if pos is not None and partial_leg not in {"", "none"}:
                    leg_exit_px = xs.get("leg_exit_px") if isinstance(xs.get("leg_exit_px"), dict) else {}
                    _apply_double_strangle_leg_closed_to_pos(pos, partial_leg, float(leg_exit_px.get(partial_leg) or 0.0))
